...
- На странице `Расселение` диапазон ползунка целевой доли сельской и пригородной среды к 2050 году расширен до `-10...+10 п.п.`.
- Пресеты остаются калиброванными по рядам статьи; изменение затрагивает только ручную настройку диапазона.
- Правка отображения KPI `изменение СКР к 2050` не выполнялась: пользователь снял этот пункт.

## QA

- Проверка `scripts/check_settlement_article_scenarios.py` теперь фиксирует диапазон ползунка `-10...+10`.
