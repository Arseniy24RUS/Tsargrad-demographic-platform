...
- `docs/family.html` — `Семья`: браки, разводы, коэффициенты на 1000 населения, индекс разводимости, локальная SVG-картограмма субъектов и сценарная оценка семейной устойчивости.
...
Playwright открывает главную и все восемь разделов, блокирует внешние запросы, делает desktop/mobile-скриншоты, проходит ключевые интеракции и full-page visual QA для `index.html`, `family.html`, `abortions.html` и ВЦИОМ-блока на `skr.html`. СКР проверяется на `skr.html` с drag-лаг-зоной, а не как старая страница на `index.html`.
