...
- `docs/skr.html` — `СКР`: Россия / федеральный округ / субъект РФ, помесячный факт и прогноз СКР до 2050, целевая траектория, краткий и подробный режимы, drag-управление лаг-зоной на графике.
...
- `docs/estate.html` — `Усадьба`: 3D-конструктор многопоколенной усадьбы на локальном Three.js, расчёт площади и стоимости.
...
- `docs/family.html` — `Семья`: браки, разводы, коэффициенты на 1000 населения и индекс разводимости по территориям.
- `docs/abortions.html` — `Аборты`: мониторинг прерываний беременности, показатели на 1000 женщин и на 100 родов, сценарная оценка сохранённых рождений.
...
Playwright открывает главную и все восемь разделов, блокирует внешние запросы, делает desktop/mobile-скриншоты, проходит ключевые интеракции и проверяет, что СКР работает на `skr.html` с drag-лаг-зоной, а не как старая страница на `index.html`.
