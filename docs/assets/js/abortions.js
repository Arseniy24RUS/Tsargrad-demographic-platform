(() => {
  ...
  function renderMap() {
    ...
    const hover = features.map((f, i) => {
      ...
      return [`<b>${f.properties.territory_name}</b>`, `Год: ${year}`, `Число: ${fmtInt(r.abortions)}`, `На 1000 женщин: ${fmt2(r.abortions_per_1000_women_15_49)}`, `На 100 родов: ${fmt2(r.abortions_per_100_births)}`].join('<br>');
    });
    const trace = {
      type: 'choropleth',
      geojson: state.geo,
    ...
    Plotly.react('abortionsMap', [trace], layout, plotConfig());
  }
